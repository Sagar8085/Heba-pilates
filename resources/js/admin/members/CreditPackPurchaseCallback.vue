<template>
    <div>
        <PageHeader title="Please Wait" subtitle="Purchase is being processed, please don't leave or refresh this page" icon="event" />
    </div>
</template>
...
export default {
    ...
    mounted() {
        axios.post('/api/admin/members/' + this.$route.params.id + '/credit-packs/callback', {
            stripeSessionID: this.$route.query.session_id,
            creditPackId: this.$route.query.creditPackId
        }).then(response => {
            console.log(response.data);
            window.location = '/admin/members/' + this.$route.params.id + '/?credit-packs=complete';
        }).catch(error => {
            console.log('ERROR');
        });
        console.log('success!')

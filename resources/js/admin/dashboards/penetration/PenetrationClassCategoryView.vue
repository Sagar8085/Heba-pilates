            ...
            <div class="wrapper">
                <div class="page-header__col">
                    <h1 class="page-header__title">
                        ...
                        Classes Penetration | {{ this.category.name }}
                    </h1>

                    <h2 class="page-header__sub">Class Category Penetration Stats</h2>
                </div>
            </div>
        </section>
                    ...
                    <div class="graph">
                        <div class="graph__header">
                            <div class="graph__header__content">
                                ...
                                <span class="graph__header__subtitle">Views for classes within this category over the last 30 days</span>
                            </div>
                        </div>
            ...
            <section class="row page-section">
                <div class="twelve-lg columns">
                    <div class="graph">
                        ...
                        <router-link :to="'/admin/dashboard/penetration/class/' + this.$route.params.id + '/members'" class="button button--outlines" style="margin: .5rem 2rem 2rem 2rem;">View Member List</router-link>
                    </div>
                </div>
            </section>
            ...
            <table class="list list--no-top list--new list--side list--with-hover">
                ...
                <tbody>
                    <tr v-for="item in this.topClasses">
                        <td><router-link :to="'/admin/dashboard/penetration/class/' + $route.params.id + '/' + item.on_demand_id">{{ item.class.name }}</router-link></td>
                        <td>{{item.total}}</td>
                    </tr>
                </tbody>
            </table>
...
export default {
    ...
    methods: {
         ...
         */
        fetchClassList() {
            axios.get('/api/admin/dashboard/penetration/classes/' + this.$route.params.id + '/top-classes').then(response => {
         ...
         */
        loadViewsGraph() {
            axios.get('/api/admin/dashboard/penetration/popularity?type=class-category&id=' + this.$route.params.id).then(response => {
                this.popularityData = {
         ...
         */
        loadAgeDemographics() {
            axios.get('/api/admin/dashboard/penetration/demographics/age?type=class-category&id=' + this.$route.params.id).then(response => {
        ...
        loadGenderDemographics() {
            axios.get('/api/admin/dashboard/penetration/demographics/gender?type=class-category&id=' + this.$route.params.id).then(response => {
                this.genderData = {
                    labels: response.data.legend,
                    datasets: [{
                        data: response.data.data,

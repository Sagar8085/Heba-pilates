...
class VirtualTrainingController extends Controller
{
    public function coaches()
    {
        $coaches = User::onlyTrainers()->with('trainer')->latest()->get();
    ...
    public function bookSession(Request $request): JsonResponse
    {
        foreach ($request->bookings as $slot) {
            $memberPackage = MemberPackage::where('id', $request->package_id)->first();
            ...
            $session = Session::create([
                ...
                'user_package_id' => $request->package_id,
            ]);
    ...
    public function singleSession(Session $session): JsonResponse
    {
        ...
        return response()->json($session->load('link'));
    }

    public function singleMemberPackage(MemberPackage $package): JsonResponse
    {
        $package = MemberPackage::where('id', $package->id)
    ...
    public function storeFeedback(Session $session, Request $request): JsonResponse
    {
        $session->update([
